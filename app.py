...
def login_user(name, password):
    ...
    try:
        ...
        if user:
            stored_pw = user[2]  # Could be str or bytes
            if isinstance(stored_pw, bytes):
                # Convert bytes -> str (assume it's base64 in raw bytes)
                stored_pw = stored_pw.decode("utf-8")

            try:
                stored_pw_bytes = base64.b64decode(stored_pw)
            except binascii.Error:
                st.error("Error: Password hash is not valid base64 (Incorrect padding). Please fix the password.")
                return None
...
def register_user(name, password):
    ...
    try:
        ...
        st.error("Hospital name already exists")
    except sqlite3.Error as e:
        st.error(f"Database error (register): {str(e)}")
    return False
        ...
        if auth_type == "Login":
            if st.button("Login 🚀"):
                ...
                if user:
                    ...
                    st.session_state.user_name = user[1]
                    st.session_state.is_admin = bool(user[4])
                    st.success("Login successful! Interact again or refresh to see new state.")
                    st.stop()
                else:
                    st.error("Invalid credentials or database error.")
        else:
...
def hospital_dashboard():
    ...
    if st.sidebar.button("Logout"):
        st.session_state.clear()
        st.success("Logged out. Refresh or interact again to update view.")
        st.stop()
                    ...
                    if st.button("Run Fraud Detection 🔍"):
                        with st.spinner("Analyzing data..."):
                            try:
                                ...
                                fraud_count = (df["Prediction"] != "No Fraud").sum()
                                ...
                                # Insert into 'datasets' & 'predictions'
                                c.execute("""
                                    INSERT INTO datasets 
                                    (hospital_id, upload_date, data_path, predictions_path)
                                    VALUES (?,?,?,?)
                                ...
                                c.execute("""
                                    ...
                                    (dataset_id, total_cases, fraud_count, non_fraud_count)
                                    VALUES (?,?,?,?)
                                """, (dataset_id, len(df), fraud_count, len(df) - fraud_count))
                                conn.commit()
        ...
        try:
            ...
            if datasets:
                ...
                if pred_data:
                    ...
                    if st.checkbox("Show detailed data"):
                        df = pd.read_csv(selected[4])
                        st.dataframe(df)

                        # Breakdown of each fraud label
                        st.markdown("### Detailed Fraud Breakdown")
    ...
    # 3) Account Info (includes change password logic)
    with tab3:
        st.subheader("Account Information")
        user_data = c.execute("""
            SELECT name, registered_date 
        ...
        with st.form("change_password_form"):
            ...
            if submitted:
                if new_password != confirm_password:
                    ...
                    # We must verify old password is correct first
                    user = login_user(st.session_state.user_name, old_password)
                    if user:
...
def admin_dashboard():
    ...
    tab1, tab2, tab3 = st.tabs(["🏥 Hospitals", "📈 National Analytics", "🔍 Detailed View"])
    ...
    with tab2:
        st.subheader("National Fraud Overview")
        try:
            # Load all predictions_path from datasets table
            c.execute("SELECT predictions_path FROM datasets")
            all_paths = c.fetchall()

            # Gather all CSVs that exist
            all_frames = []
            for row in all_paths:
                path = row[0]
                if path and os.path.exists(path):
            ...
            if all_frames:
                ...
                if "Prediction" in combined_df.columns:
                    # Calculate total national cases & total fraud
                    total_cases = len(combined_df)
                    total_fraud = combined_df[combined_df["Prediction"] != "No Fraud"].shape[0]
                    ...
                    if total_cases > 0:
                        ...
                        if not fraud_counts.empty:
                            ...
                            st.info("No fraud predictions found in the combined data.")
                    else:
                        st.info("No data available for analysis.")
                else:
                    st.info("No 'Prediction' column found in the combined data.")
            else:
                st.info("No data files available for national fraud analysis.")
        except sqlite3.Error as e:
            st.error(f"Database error (national analytics): {str(e)}")
        ...
        try:
            ...
            for (hosp_id,) in all_hosp_ids:
                ...
                for ds in ds_paths:
                    ...
                    if predictions_path and os.path.exists(predictions_path):
                        df_tmp = pd.read_csv(predictions_path)
                        # Keep only rows that are flagged as anything but "No Fraud"
                        fraud_rows = df_tmp[df_tmp["Prediction"] != "No Fraud"]
                        if not fraud_rows.empty:
                            all_data.append(fraud_rows)
